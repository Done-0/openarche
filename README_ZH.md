...
| 字段 | 默认值 | 说明 |
|------|--------|------|
| `embedding.provider` | `local` | `local` 或 `openai`。切换后需运行 `/openarche:reindex` |
| `embedding.localModel` | `Xenova/multilingual-e5-small` | 本地模型，支持 100+ 语言，~120MB |
| `embedding.remoteProvider` | `""` | 远程 embedding 提供商（如 `openai`）|
| `embedding.remoteModel` | `""` | 远程 embedding 模型（如 `text-embedding-3-small`）|
| `embedding.remoteApiKey` | `""` | OpenAI embedding API Key（使用远程时填写）|
| `retrieval.threshold` | `0.72` | 向量相似度阈值，调高减少噪音，调低增加召回 |
| `retrieval.topK` | `5` | 每次最多注入的种子记忆数 |
